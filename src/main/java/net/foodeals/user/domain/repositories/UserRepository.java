...
public interface UserRepository extends BaseRepository<User, Integer> {
	...
	@Query("SELECT u FROM User u WHERE u.role.name = 'COLLABORATOR' AND u.deletedAt IS NULL")
	Page<User> findAllCollaborators(Pageable pageable);
	
	@Query("SELECT u FROM User u WHERE u.role.name = 'COLLABORATOR' AND u.deletedAt IS NOT NULL")
	Page<User> findAllArchivedCollaborators(Pageable pageable);

	
	@Query("SELECT COUNT(u) FROM User u WHERE u.role.name = 'COLLABORATOR'")
	long countCollaborators();

	@Query("SELECT COUNT(u) FROM User u WHERE u.role.name = 'COLLABORATOR' AND u.status = 'ACTIVE'")
	long countRecrutement();

	@Query("SELECT COUNT(u) FROM User u WHERE u.role.name = 'COLLABORATOR' AND u.source = 'PRESTATAIRE'")
	long countPrestataires();
}
