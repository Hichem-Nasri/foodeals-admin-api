...
import net.foodeals.location.application.dtos.requests.LocationRequest;
import net.foodeals.location.application.dtos.responses.LocationResponse;
...
class AddressServiceImpl implements AddressService {
    ...
    public LocationResponse createLocation(LocationRequest request) {
        Country country = this.countryService.findByName(request.country());
        if (country == null) {
            country =new Country();
            country.setName(request.country().toLowerCase());
            country =countryService.save(country);
        }
        State state =this.stateService.findByName(request.stateName());
        if(state ==null) {
        	state=new State();
        ...
        Region region=regionService.findByName(request.regionName());
        if(region==null) {
        	region =new Region();
        ...
        City city =cityService.findByName(request.cityName());
        	...
        	state = stateService.save(state);
        LocationResponse response =new LocationResponse(country.getName(), city.getName(), region.getName(), state.getName());
